...
<?php
    if (isset($_GET['syllabus_id']) && isset($_GET['question_id']) && isset($_GET['id'])) {
        ...
        if($exam_from == "reguler-student"){
            $assigned_question = $conn->query("SELECT ID FROM Students_Answers WHERE Student_ID = ".$_SESSION['ID']." AND Syllabus_ID = $syllabus_id AND Question_ID = $Question_id ORDER BY ID ASC");

            $question_first = $assigned_question->fetch_assoc()['ID'] + 1;
            $assigned_question = $conn->query("SELECT Question_ID FROM Students_Answers WHERE ID = ".$question_first." AND Student_ID = ".$_SESSION['ID']." ORDER BY ID ASC");
