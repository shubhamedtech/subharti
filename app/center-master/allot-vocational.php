<?php
  if(isset($_POST['university_id']) && isset($_POST['id']) && isset($_POST['counsellor']) && isset($_POST['course_type'])){
    ...
    if(empty($counsellor) || empty($university_id) || empty($id) || empty($course_types)){
    ...
    if($university_id == 48){ 
      ...
      foreach ($fees as $sub_course_id=>$value){
        ...
        foreach($value as $duration=>$fee){
          ...
          if(empty($fee)){
            ...
            if($check->num_rows==0){
              $conn->query("DELETE FROM Center_Sub_Courses WHERE User_ID IN (SELECT Sub_Center FROM Center_SubCenter WHERE Center = $id) AND Duration = '$duration' AND Sub_Course_ID = $sub_course_id");
              $check = $conn->query("SELECT ID FROM Students WHERE Duration = '$duration' AND Sub_Course_ID = $sub_course_id AND Added_For = $id");
              if($check->num_rows==0){
                $conn->query("DELETE FROM Center_Sub_Courses WHERE User_ID = $id AND Duration = '$duration' AND Sub_Course_ID = $sub_course_id");
              }
            }
          }else{
            $conn->query("DELETE FROM Center_Sub_Courses WHERE User_ID = $id AND Duration = '$duration' AND Sub_Course_ID = $sub_course_id");
            $allot = $conn->query("INSERT INTO Center_Sub_Courses (`Fee`, `User_ID`, `Duration`, `Sub_Course_ID`, `Course_ID`, `University_ID`) VALUES ($fee, $id, '$duration', $sub_course_id, $courseId, $university_id)");
          }
        }
      }
    }else{
      ...
      foreach ($fees as $sub_course_id=>$fee){
        ...
        $allot = $conn->query("INSERT INTO Center_Sub_Courses (`Fee`, `Duration`, `User_ID`, `Course_ID`, `Sub_Course_ID`, `University_ID`) VALUES ($fee, 1, $id, $course_id, $sub_course_id, $university_id)");
      }
    }
