...
if (isset($_FILES['file'])) {
  ...
  if ($_SESSION['university_id'] == 48) {
    $header = array('Scheme','Course', 'Specialization', 'Category', 'Duration', 'Subject Code', 'Subject Name', 'Type (Theory/Practical)', 'Credit','Minimum Marks', 'Maximum Marks','Remark');
  } else {
    ...
    for ($i = 0; $i < $totalSheet; $i++) {
      ...
      if ($_SESSION['university_id'] == 48) {
        foreach ($reader as $row) {
          ...
          // echo "<pre>"; 
          // echo "SELECT ID FROM Syllabi WHERE University_ID = " . $_SESSION['university_id'] . " AND Course_ID = $course_id AND Sub_Course_ID = $sub_course_id AND Scheme_ID = $scheme_id AND Code = '" . $subject_code . "' AND Semester='".$duration."'";
          $check = $conn->query("SELECT ID FROM Syllabi WHERE University_ID = " . $_SESSION['university_id'] . " AND Course_ID = $course_id AND Sub_Course_ID = $sub_course_id AND Scheme_ID = $scheme_id AND Code = '" . $subject_code . "'  AND Semester='".$duration."'");
          ...
          $check = $conn->query("SELECT ID FROM Syllabi WHERE University_ID = " . $_SESSION['university_id'] . " AND Course_ID = $course_id AND Sub_Course_ID = $sub_course_id AND Scheme_ID = $scheme_id AND Name = '" . $subject_name . "' AND Semester='".$duration."'");
          if ($check->num_rows > 0) {
            $export_data[] = array_merge($row, ['Subject Name already exists!']);
            continue;
          }
        //  echo "<pre>"; 
        //  echo "INSERT INTO `Syllabi`(`University_ID`, `Course_ID`, `Sub_Course_ID`, `Scheme_ID`, `Code`, `Name`, `Paper_Type`, `Credit`, `Min_Marks`, `Max_Marks`,`Semester`) VALUES (" . $_SESSION['university_id'] . ", " . $course_id . ", " . $sub_course_id . ", " . $scheme_id . ",'" . $subject_code . "', '" . $subject_name . "', '" . $paper_type . "', " . $credit . ", " . $min_marks . ", " . $max_marks . ",'" . $duration . "')";
