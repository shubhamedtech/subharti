...
if (isset($_GET['id'])) {
  ...
  $student = $conn->query("SELECT  Students.ID AS Stu_id,Universities.Short_Name as University, Sub_Courses.Name as Sub_Cour_name, Students.Duration as no_semester, Courses.Short_Name as Course, DATE_FORMAT(Students.DOB, '%d-%m-%Y') as DOB,  RIGHT(CONCAT('000000', Students.ID), 6) as Student_Table_ID, Students.Unique_ID as Student_ID,JSON_UNQUOTE(JSON_EXTRACT(Student_Ledgers.Fee,'$.Paid'))AS Invoiced_Amount,Student_Ledgers.Duration,CONCAT(IF(Students.Unique_ID='' OR Students.Unique_ID IS NULL, RIGHT(CONCAT('000000', Students.ID), 6), Students.Unique_ID), ' . ', TRIM(CONCAT(Students.First_Name, ' ', Students.Middle_Name, ' ', Students.Last_Name))) as Unique_ID,DATE_FORMAT(Student_Ledgers.Date, '%d-%m-%Y') as Date,Student_Ledgers.Source,Student_Ledgers.Transaction_ID FROM Student_Ledgers LEFT JOIN Payments ON Student_Ledgers.Transaction_ID=Payments.Transaction_ID LEFT JOIN Students ON Student_Ledgers.Student_ID=Students.ID LEFT JOIN Sub_Courses ON
Students.Sub_Course_ID = Sub_Courses.ID LEFT JOIN Courses ON Students.Course_ID = Courses.ID LEFT JOIN Universities ON Students.University_ID = Universities.ID WHERE Student_Ledgers.ID= $id");
  $details = $student->fetch_assoc();
  $student_id = !empty($details['Student_ID']) ? $details['Student_ID'] : $details['Student_Table_ID'];

  if ($details['Source'] == "Wallet") {
    $wallet_query = $conn->query("SELECT RIGHT(CONCAT('000000', Wallet_Payments.ID), 6) as ID,Wallet_Payments.Payment_Mode,Wallet_Payments.Bank, Wallet_Payments.Gateway_ID,Wallet_Payments.Type, Wallet_Payments.Transaction_Date FROM Wallet_Payments WHERE Transaction_ID = '" . $details['Transaction_ID'] . "'");
    $wallet_details = $wallet_query->fetch_assoc();
    $details = array_merge($details, $wallet_details);
  } else {
    $payment_query = $conn->query("SELECT RIGHT(CONCAT('000000', Payments.ID), 6) as ID,Payments.Bank,Payments.Payment_Mode, Payments.Gateway_ID,Payments.Type, Payments.Transaction_Date FROM Payments WHERE Transaction_ID = '" . $details['Transaction_ID'] . "'");
    $payment_details = $payment_query->fetch_assoc();
    $details = array_merge($details, $payment_details);
  }
  ...
  $details['no_semester'] = $_GET['duration'];
