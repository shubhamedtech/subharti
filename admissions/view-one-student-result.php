          ...
          <table class="table table-striped">
            ...
            <tbody class="text-center">
              ...
              if (isset($_GET['student_id'])) {
                $Syllabi_id = $_GET['Syllabi_id'];
                $attempt_question = 0;
                $correct_question = 0;
                $total_mark = 0;
                $status =  "A";
                $exam_submits = $conn->query("SELECT * FROM Exam_Attempts WHERE Student_ID = '" . $_GET['student_id'] . "'");

                while ($exam_submit = $exam_submits->fetch_assoc()) {
                  $student_answers = $conn->query("SELECT * FROM Students_Answers WHERE Student_ID = '" . $exam_submit['Student_ID'] . "'AND Syllabus_ID = ".$Syllabi_id. " AND Date_Sheet_ID = '" . $exam_submit['Date_Sheet_ID'] . "' ");
                  $attempt_question = 0;
                  $correct_question = 0;
                  $total_mark = 0;
                ...
                $exam_sub = $conn->query("SELECT Name FROM Syllabi WHERE ID = '".$_GET['Syllabi_id']."' ");
              ?>
                  <tr>
                    <td><?= $exam_sub->fetch_assoc()['Name'] ?></td>
